      ...
      <div className="bg-white rounded-lg shadow-2xl max-w-2xl w-full max-h-[90vh] flex flex-col border border-rose-100" onClick={e => e.stopPropagation()}>
        <form onSubmit={handleSubmit} className="p-8 flex flex-col flex-grow">
          <div className="flex justify-between items-center mb-6">
            <h2 className="text-2xl font-bold font-serif text-rose-700">{collection && 'id' in collection ? 'Edit Collection' : 'Add New Collection'}</h2>
            <button type="button" onClick={onClose} className="text-rose-400 hover:text-rose-600">
              <XIcon className="h-6 w-6" />
            </button>
          </div>
          <div className="mb-4">
            <label htmlFor="name" className="block text-rose-700 font-medium mb-1">Collection Name</label>
          ...
          <p className="text-rose-700 font-medium mb-2">Select Books</p>
          ...
          <div className="mt-8 text-right">
            ...
            <button type="submit" className="px-6 py-2 bg-rose-500 text-white font-bold rounded-md hover:bg-rose-400">Save Collection</button>
          </div>
        </form>
      </div>
  );
